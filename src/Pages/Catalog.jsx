      ...
      const getCategories = async () => {
          try {
              ...
              if (res?.data?.success && res?.data?.data) {
                  ...
                  // Ensure that category is found
                  if (category) {
                      setCategoryId(category._id);
                  } else {
                      console.error("Category not found for catalogName:", catalogName);
                      setCategoryId(""); // Optionally reset or handle the case where no category is found
                  }
              } else {
          ...
          <div className=" mx-auto box-content w-full max-w-maxContentTab px-4 py-12 lg:max-w-maxContent">
            ...
            <div className="my-4 flex border-b border-b-richblack-600 text-sm">
              <p
                className={`px-4 py-2 ${
                  active === 1
                    ? "border-b border-b-caribbeangreen-50 text-caribbeangreen-100"
                    : "text-richblack-50"
                } cursor-pointer`}
                onClick={() => setActive(1)}
              >
